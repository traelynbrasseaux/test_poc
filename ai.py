...
def yolo_detect(image_path, confidence_threshold=None):
    ...
    for box in results.boxes:
        ...
        detections.append({'label': label, 'confidence': conf, 'bbox': xyxy.tolist()})
    return detections
...
def compare():
    logger.debug('Entered compare function')
    if 'reference' not in request.files or 'current' not in request.files:
        logger.debug('Missing image files in request.files')
        return jsonify({'error': 'Missing image files'}), 400
    reference = request.files['reference']
    current = request.files['current']
    logger.debug(f'reference filename: {reference.filename}, current filename: {current.filename}')
    ...
    ref_path = os.path.join(upload_folder, f'ref_{timestamp}.jpg')
    curr_path = os.path.join(upload_folder, f'curr_{timestamp}.jpg')
    logger.debug(f'ref_path: {ref_path}, curr_path: {curr_path}')
    try:
        ...
        logger.debug(f'ref_detections: {ref_detections}')
        logger.debug(f'curr_detections: {curr_detections}')
        ...
        ref_vis_path = os.path.join(upload_folder, f'ref_vis_{timestamp}.jpg')
        curr_vis_path = os.path.join(upload_folder, f'curr_vis_{timestamp}.jpg')
        ...
        for color, text in overlay_legend:
            ...
            legend_y += 35
        overlay_path = os.path.join(upload_folder, f'overlay_{timestamp}.jpg')
        cv2.imwrite(overlay_path, overlay_img)
        logger.debug('Overlay image saved')
        return jsonify({
            'issues': issues,
